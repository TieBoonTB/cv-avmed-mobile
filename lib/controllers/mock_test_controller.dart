...
class MockTestController extends BaseTestController {
  ...
  bool processDetectionResults(
    ...
    // For mock controller, use default service (backward compatibility)
    final detections = detectionsByService['default'] ?? [];

    print(
        'Processing ${detections.length} detections for step: ${currentStep.label}');
    for (final detection in detections) {
      print(
          '  Found: ${detection.label} with confidence ${detection.confidence}');
    }
    ...
    for (final detection in detections) {
      if (detection.label == currentStep.targetLabel &&
        ...
        print(
            '  ✅ Match found: ${detection.label} (${detection.confidence}) >= ${currentStep.confidenceThreshold}');
        return true;
      }
    }
  ...
  Future<void> onStepEnd(TestStep step, bool isSuccess) async {
    ...
    print(
        '  Progress: ${step.detectedFrameCount}/${step.targetFrameCount} detections');

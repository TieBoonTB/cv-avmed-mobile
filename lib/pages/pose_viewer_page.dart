...
class PoseViewerPageState extends State<PoseViewerPage> {
  ...
  void _zoomBy(double factor) {
    ...
    final focal = Offset(MediaQuery.of(context).size.width / 2,
        MediaQuery.of(context).size.height / 2);

    // Translate to focal, scale, translate back
    final translationToFocal = Matrix4.identity()
    ...
    final newMatrix =
        translationBack * scaleMatrix * translationToFocal * matrix;
  ...
  Future<void> _loadAssetImage(String assetPath) async {
    ...
    try {
      ...
      final results =
          await _service?.processFrame(data, _image!.height, _image!.width) ??
              [];
      setState(() {
        _landmarks = results;
  ...
  void _startInferenceTimer() {
    ...
    _inferenceTimer =
      ...
      try {
        ...
        final bytes = CameraImageUtils.convertCameraImageToBytes(lastImage,
            isFrontCamera: _cameraFeedController.isFrontCamera);
        if (bytes.isEmpty) return;

        final results = await _service?.processFrame(
                bytes, lastImage.height, lastImage.width) ??
            [];
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            icon: Icon(_cameraFeedController.isFrontCamera
                ? Icons.camera_front
                : Icons.camera_rear),
            onPressed: () async {
      ...
      body: Padding(
        ...
        child: _showCameraFeed
            ? _buildCameraFeedView()
            : _buildPoseViewerWithImage(),
      ),
    );
  }
      ...
      children: [
        Row(
          children: [
              ...
              child: DropdownButtonFormField<String>(
                ...
                items: _assetOptions
                    .map((a) => DropdownMenuItem(
                        value: a, child: Text(a.split('/').last)))
                    .toList(),
            ...
            const SizedBox(width: 12),
            ElevatedButton(
              onPressed: _selectedAsset == null || _loading
                  ? null
                  : () => _loadAssetImage(_selectedAsset!),
              child: _loading
                  ? const SizedBox(
                      width: 16,
                      height: 16,
                        ...
                        child: Column(
                          children: [
                            ...
                            Container(
                              ...
                              child: Text(
                                  'x${_currentScale.toStringAsFixed(2)}',
